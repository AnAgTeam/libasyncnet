...
TEST_CASE("NetworkRequestor copy, move") {
	NetworkRequestor requestor(2);
	NetworkRequestor requestor2 = requestor;
	NetworkRequestor requestor3 = std::move(requestor);
}
...
TEST_CASE("NetworkRequestor request") {
	...
	auto worker = [](NetworkRequestor& requestor, std::string_view url) -> coro::task<void> {
		curlpp::Easy easy;
...
TEST_CASE("NetworkRequestor custom pool") {
	...
	NetworkRequestor requestor(2, pool);

	auto worker = [](NetworkRequestor& requestor, std::shared_ptr<coro::thread_pool> pool) -> coro::task<void> {
		...
		REQUIRE_THROWS_AS(co_await requestor.perform_handle(easy), NetworkRequestError);
